...
public class Region implements IShape {
	...
	public boolean touches(final Region r) {
		...
		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++) {
			if ((m_pLow[cIndex] > r.m_pLow[cIndex] - SpatialIndex.EPSILON && m_pLow[cIndex] < r.m_pLow[cIndex]
					+ SpatialIndex.EPSILON)
					|| (m_pHigh[cIndex] > r.m_pHigh[cIndex]
							- SpatialIndex.EPSILON && m_pHigh[cIndex] < r.m_pHigh[cIndex]
							+ SpatialIndex.EPSILON))
				return true;
		}
		return false;
	...
	public boolean touches(final Point p) {
		if (m_pLow.length != p.m_pCoords.length)
			throw new IllegalArgumentException(
					"touches: Shape has the wrong number of dimensions.");
		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++) {
			if ((m_pLow[cIndex] > p.m_pCoords[cIndex] - SpatialIndex.EPSILON && m_pLow[cIndex] < p.m_pCoords[cIndex]
					+ SpatialIndex.EPSILON)
					|| (m_pHigh[cIndex] > p.m_pCoords[cIndex]
							- SpatialIndex.EPSILON && m_pHigh[cIndex] < p.m_pCoords[cIndex]
							+ SpatialIndex.EPSILON))
				return true;
		}
		return false;
	...
	// Returns the margin of a region. It is calcuated as the sum of 2^(d-1) *
