...
abstract class Node implements INode {
	...
	// Abstract methods
	//
	protected abstract Node chooseSubtree(Region mbr, int level,
			Stack pathBuffer);
	...
	protected void insertEntry(byte[] pData, Region mbr, int id)
		...
		if (m_children == 0) {
			...
			for (int cDim = 0; cDim < m_pTree.m_dimension; cDim++) {
				...
				for (int cChild = 0; cChild < m_children; cChild++) {
					m_nodeMBR.m_pLow[cDim] = Math.min(m_nodeMBR.m_pLow[cDim],
							m_pMBR[cChild].m_pLow[cDim]);
					m_nodeMBR.m_pHigh[cDim] = Math.max(m_nodeMBR.m_pHigh[cDim],
							m_pMBR[cChild].m_pHigh[cDim]);
				}
			}
		}
	}

	protected boolean insertData(byte[] pData, Region mbr, int id,
			Stack pathBuffer, boolean[] overflowTable) {
		if (m_children < m_capacity) {
			...
			return adjusted;
		} else if (m_pTree.m_treeVariant == SpatialIndex.RtreeVariantRstar
				&& !pathBuffer.empty() && overflowTable[m_level] == false) {
			...
			for (cIndex = 0; cIndex < lReinsert; cIndex++) {
				m_pTree.insertData_impl(reinsertdata[cIndex],
						reinsertmbr[cIndex], reinsertid[cIndex], m_level,
						overflowTable);
			}
			return true;
		} else {
			...
			if (pathBuffer.empty()) {
				...
				r.insertEntry(null, (Region) n.m_nodeMBR.clone(),
						n.m_identifier);
				r.insertEntry(null, (Region) nn.m_nodeMBR.clone(),
						nn.m_identifier);
				m_pTree.writeNode(r);
	...
	protected void reinsertData(byte[] pData, Region mbr, int id,
			ArrayList reinsert, ArrayList keep) {
		...
		int cReinsert = (int) Math.floor((m_capacity + 1)
				* m_pTree.m_reinsertFactor);
		int cCount;
	...
	protected void rtreeSplit(byte[] pData, Region mbr, int id,
			ArrayList group1, ArrayList group2) {
		int cChild;
				...
				for (cChild = 0; cChild < m_capacity + 1; cChild++) {
					if (mask[cChild] == false) {
						...
						if (d > m) {
							...
							md2 = d2;
							sel = cChild;
							if (m_pTree.m_treeVariant == SpatialIndex.RtreeVariantLinear
									|| m_pTree.m_treeVariant == SpatialIndex.RtreeVariantRstar)
								break;
						}
					}
				}
	...
	protected void rstarSplit(byte[] pData, Region mbr, int id,
			ArrayList group1, ArrayList group2) {
		...
		m_pMBR[m_capacity] = mbr;
		m_pIdentifier[m_capacity] = id;
		int nodeSPF = (int) (Math.floor((m_capacity + 1)
				* m_pTree.m_splitDistributionFactor));
		...
		case SpatialIndex.RtreeVariantRstar:
			for (cDim = 0; cDim < m_pTree.m_dimension; cDim++) {
				...
				for (cChild = 1; cChild < m_capacity + 1; cChild++) {
					if (m_pMBR[cChild].m_pLow[cDim] > m_pMBR[greatestLower].m_pLow[cDim])
						greatestLower = cChild;
					if (m_pMBR[cChild].m_pHigh[cDim] < m_pMBR[leastUpper].m_pHigh[cDim])
						leastUpper = cChild;
					leastLower = Math.min(m_pMBR[cChild].m_pLow[cDim],
							leastLower);
					greatestUpper = Math.max(m_pMBR[cChild].m_pHigh[cDim],
							greatestUpper);
				}
				width = greatestUpper - leastLower;
				if (width <= 0)
					width = 1;
				double f = (m_pMBR[greatestLower].m_pLow[cDim] - m_pMBR[leastUpper].m_pHigh[cDim])
						/ width;
				if (f > separation) {
					i1 = leastUpper;
					i2 = greatestLower;
	...
	protected void condenseTree(Stack toReinsert, Stack pathBuffer) {
		...
		if (pathBuffer.empty()) {
			...
			if (m_level != 0 && m_children == 1) {
				...
				m_pTree.writeNode(n);
				m_pTree.m_stats.m_nodesInLevel
						.remove(m_pTree.m_stats.m_nodesInLevel.size() - 1);
				m_pTree.m_stats.m_treeHeight -= 1;
				// HACK: pending deleteNode for deleted child will decrease
				// nodesInLevel, later on.
				m_pTree.m_stats.m_nodesInLevel.set(
						m_pTree.m_stats.m_treeHeight - 1, new Integer(2));
			}
		} else {
			int cParent = ((Integer) pathBuffer.pop()).intValue();
			Index p = (Index) m_pTree.readNode(cParent);
			...
			for (child = 0; child != p.m_children; child++) {
				if (p.m_pIdentifier[child] == m_identifier)
					break;
			}
			if (m_children < minimumLoad) {
				...
				for (int cDim = 0; cDim < m_pTree.m_dimension; cDim++) {
					...
					for (int cChild = 0; cChild < p.m_children; cChild++) {
						p.m_nodeMBR.m_pLow[cDim] = Math.min(
								p.m_nodeMBR.m_pLow[cDim],
								p.m_pMBR[cChild].m_pLow[cDim]);
						p.m_nodeMBR.m_pHigh[cDim] = Math.max(
								p.m_nodeMBR.m_pHigh[cDim],
								p.m_pMBR[cChild].m_pHigh[cDim]);
					}
				}
			}
