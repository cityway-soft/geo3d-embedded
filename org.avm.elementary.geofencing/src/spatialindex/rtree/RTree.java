...
public class RTree implements ISpatialIndex {
		...
		if (var != null) {
			if (!(var instanceof Integer))
				throw new IllegalArgumentException(
						"Property IndexIdentifier must an Integer");
			...
			try {
				...
				throw new IllegalStateException(
						"initOld failed with IOException");
			}
		} else {
			try {
				initNew(ps);
				...
				System.err.println(e);
				throw new IllegalStateException(
						"initNew failed with IOException");
			}
	...
	public void pointLocationQuery(final IShape query, final IVisitor v) {
		...
		if (query instanceof Point) {
			...
			throw new IllegalArgumentException(
					"pointLocationQuery: IShape can be Point or Region only.");
		}
		rangeQuery(SpatialIndex.IntersectionQuery, r, v);
	}

	public void nearestNeighborQuery(int k, final IShape query,
			final IVisitor v, final INearestNeighborComparator nnc) {
		if (query.getDimension() != m_dimension)
			throw new IllegalArgumentException(
			...
			while (queue.size() != 0) {
				...
				if (first.m_pEntry instanceof Node) {
					...
					for (int cChild = 0; cChild < n.m_children; cChild++) {
						IEntry e;
						if (n.m_level == 0) {
							e = new Data(n.m_pData[cChild], n.m_pMBR[cChild],
									n.m_pIdentifier[cChild]);
						} else {
							e = (IEntry) readNode(n.m_pIdentifier[cChild]);
						}
						NNEntry e2 = new NNEntry(e, nnc.getMinimumDistance(
								query, e));
						// Why don't I use a TreeSet here? See comment above...
						int loc = Collections.binarySearch(queue, e2,
								new NNEntryComparator());
						if (loc >= 0)
							queue.add(loc, e2);
						else
							queue.add((-loc - 1), e2);
	...
	public PropertySet getIndexProperties() {
		...
		pRet.setProperty("NearMinimumOverlapFactor", new Integer(
				m_nearMinimumOverlapFactor));
		// split distribution factor
		pRet.setProperty("SplitDistributionFactor", new Double(
				m_splitDistributionFactor));
		// reinsert factor
	...
	public boolean isIndexValid() {
		...
		while (!st.empty()) {
			...
			for (int cDim = 0; cDim < m_dimension; cDim++) {
				...
				for (int cChild = 0; cChild < e.m_pNode.m_children; cChild++) {
					tmpRegion.m_pLow[cDim] = Math.min(tmpRegion.m_pLow[cDim],
							e.m_pNode.m_pMBR[cChild].m_pLow[cDim]);
					tmpRegion.m_pHigh[cDim] = Math.max(tmpRegion.m_pHigh[cDim],
							e.m_pNode.m_pMBR[cChild].m_pHigh[cDim]);
				}
			}
			...
			if (e.m_pNode.m_level != 0) {
				for (int cChild = 0; cChild < e.m_pNode.m_children; cChild++) {
					ValidateEntry tmpEntry = new ValidateEntry(
							e.m_pNode.m_pMBR[cChild],
							readNode(e.m_pNode.m_pIdentifier[cChild]));
					if (!nodesInLevel.containsKey(new Integer(
							tmpEntry.m_pNode.m_level))) {
						nodesInLevel.put(new Integer(tmpEntry.m_pNode.m_level),
								new Integer(1));
					} else {
						int i = ((Integer) nodesInLevel.get(new Integer(
								tmpEntry.m_pNode.m_level))).intValue();
						nodesInLevel.put(new Integer(tmpEntry.m_pNode.m_level),
								new Integer(i + 1));
					}
					st.push(tmpEntry);
		...
		for (int cLevel = 0; cLevel < m_stats.m_treeHeight; cLevel++) {
			int i1 = ((Integer) nodesInLevel.get(new Integer(cLevel)))
					.intValue();
	...
	private void initNew(PropertySet ps) throws IOException {
		...
		if (var != null) {
			if (var instanceof Integer) {
				...
				if (i != SpatialIndex.RtreeVariantLinear
				...
				throw new IllegalArgumentException(
						"Property TreeVariant must be an Integer");
			}
		}
		// fill factor.
		...
		if (var != null) {
			if (var instanceof Double) {
				...
				throw new IllegalArgumentException(
						"Property FillFactor must be a Double");
			}
		}
		// index capacity.
		...
		if (var != null) {
			if (var instanceof Integer) {
				...
				throw new IllegalArgumentException(
						"Property IndexCapacity must be an Integer");
			}
		}
		// leaf capacity.
		...
		if (var != null) {
			if (var instanceof Integer) {
				...
				throw new IllegalArgumentException(
						"Property LeafCapacity must be an Integer");
			}
		}
		// near minimum overlap factor.
		...
		if (var != null) {
			if (var instanceof Integer) {
				...
				throw new IllegalArgumentException(
						"Property NearMinimumOverlapFactor must be an Integer");
			}
		}
		// split distribution factor.
		...
		if (var != null) {
			if (var instanceof Double) {
				...
				throw new IllegalArgumentException(
						"Property SplitDistriburionFactor must be a Double");
			}
		}
		// reinsert factor.
		...
		if (var != null) {
			if (var instanceof Double) {
				...
				throw new IllegalArgumentException(
						"Property ReinsertFactor must be a Double");
			}
		}
		// dimension
		...
		if (var != null) {
			if (var instanceof Integer) {
				...
				throw new IllegalArgumentException(
						"Property Dimension must be an Integer");
			}
		}
	...
	private void storeHeader() throws IOException {
		...
		for (int cLevel = 0; cLevel < m_stats.m_treeHeight; cLevel++) {
			ds.writeInt(((Integer) m_stats.m_nodesInLevel.get(cLevel))
					.intValue());
		}
		ds.flush();
		m_headerID = m_pStorageManager.storeByteArray(m_headerID, bs
				.toByteArray());
	}
	...
	protected void insertData_impl(byte[] pData, Region mbr, int id) {
		if (mbr.getDimension() != m_dimension)
			throw new Error("[DSU] mbr.getDimension() != m_dimension");
	...
	protected void insertData_impl(byte[] pData, Region mbr, int id, int level,
			boolean[] overflowTable) {
		if (mbr.getDimension() != m_dimension)
			throw new Error("[DSU] mbr.getDimension() != m_dimension");
		// assert mbr.getDimension() == m_dimension;
	...
	protected int writeNode(Node n) throws IllegalStateException {
		...
		try {
			...
			System.err.println(e);
			throw new IllegalStateException(
					"writeNode failed with InvalidPageException");
		}
		if (n.m_identifier < 0) {
			n.m_identifier = page;
			m_stats.m_nodes++;
			int i = ((Integer) m_stats.m_nodesInLevel.get(n.m_level))
					.intValue();
	...
	protected Node readNode(int id) {
		...
		try {
			...
			if (nodeType == SpatialIndex.PersistentIndex)
				...
				throw new IllegalStateException(
						"readNode failed reading the correct node type information");
			n.m_pTree = this;
			n.m_identifier = id;
			n.load(buffer);
			m_stats.m_reads++;
			...
			System.err.println(e);
			throw new IllegalStateException(
					"readNode failed with InvalidPageException");
		} catch (IOException e) {
			System.err.println(e);
	...
	protected void deleteNode(Node n) {
		try {
			...
			System.err.println(e);
			throw new IllegalStateException(
					"deleteNode failed with InvalidPageException");
		}
		m_stats.m_nodes--;
	...
	public String toString() {
		...
		s += "Utilization: " + 100 * m_stats.getNumberOfData()
				/ (m_stats.getNumberOfNodesInLevel(0) * m_leafCapacity) + "%"
				+ "\n" + m_stats;
		return s;
	}
