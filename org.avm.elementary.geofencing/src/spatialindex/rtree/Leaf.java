...
public class Leaf extends Node {
	...
	protected Leaf findLeaf(Region mbr, int id, Stack pathBuffer) {
		for (int cChild = 0; cChild < m_children; cChild++) {
			if (m_pIdentifier[cChild] == id && mbr.equals(m_pMBR[cChild]))
				return this;
		}
		return null;
	}
	...
	protected Node[] split(byte[] pData, Region mbr, int id) {
		...
		for (cIndex = 0; cIndex < g1.size(); cIndex++) {
			...
			// we don't want to delete the data array from this node's
			// destructor!
			m_pData[i] = null;
		}
	...
	protected void deleteData(int id, Stack pathBuffer) {
		...
		while (!toReinsert.empty()) {
			...
			for (int cChild = 0; cChild < n.m_children; cChild++) {
				// keep this in the for loop. The tree height might change after
				// insertions.
				...
				m_pTree.insertData_impl(n.m_pData[cChild], n.m_pMBR[cChild],
						n.m_pIdentifier[cChild], n.m_level, overflowTable);
