/* DeltaDecoder -- decodes Delta objects from external representations.
   $Id: DeltaDecoder.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
...
public abstract class DeltaDecoder {
	 ...
	 * 
	 * @param encoding
	 *            The name of the decoder to get.
	 * @param config
	 *            The configuration to use.
	 * @param in
	 *            The source of binary data.
	 * @return The new decoder.
	 * @throws NullPointerException
	 *             If any parameter is null.
	 * @throws IllegalArgumentException
	 *             If there is no appropriate decoder available.
	 */
	public static final DeltaDecoder getInstance(String encoding,
			Configuration config, InputStream in) {
		...
		try {
			...
			if (!DeltaDecoder.class.isAssignableFrom(clazz))
				throw new IllegalArgumentException(clazz.getName()
						+ ": not a subclass of " + DeltaDecoder.class.getName());
			Constructor c = clazz.getConstructor(new Class[] {
					Configuration.class, InputStream.class });
