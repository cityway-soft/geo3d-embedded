/* DeltaEncoder -- encodes Delta objects to external representations.
   $Id: DeltaEncoder.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
...
public abstract class DeltaEncoder {
	 ...
	 *             If there is no appropriate encoder available.
	 */
	public static final DeltaEncoder getInstance(String encoding,
			Configuration config, OutputStream out) {
		...
		try {
			Class clazz = Class
					.forName(System.getProperty(PROPERTY + encoding));
			if (!DeltaEncoder.class.isAssignableFrom(clazz))
				throw new IllegalArgumentException(clazz.getName()
						+ ": not a subclass of " + DeltaEncoder.class.getName());
			Constructor c = clazz.getConstructor(new Class[] {
					Configuration.class, OutputStream.class });
	 ...
	 * <code>write</code> methods <b>must</b> be presented in increasing
	 * order of their {@link Delta#getWriteOffset()} values.
	 * 
