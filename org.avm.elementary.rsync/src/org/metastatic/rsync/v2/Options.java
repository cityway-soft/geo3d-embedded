/* Options -- run-time option list.
   $Id: Options.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
...
public class Options {
	...
	public static final LongOpt[] LONGOPTS = {
			...
			new LongOpt("backup-dir", LongOpt.REQUIRED_ARGUMENT, null,
					OPT_BACKUP_DIR),
			...
			new LongOpt("copy-unsafe-links", LongOpt.NO_ARGUMENT, null,
			...
			new LongOpt("no-whole-file", LongOpt.NO_ARGUMENT, null,
					OPT_NO_WHOLE_FILE),
			...
			new LongOpt("rsync-path", LongOpt.REQUIRED_ARGUMENT, null,
					OPT_RSYNC_PATH),
			new LongOpt("cvs-exclude", LongOpt.NO_ARGUMENT, null, 'C'),
			new LongOpt("existing", LongOpt.NO_ARGUMENT, null, OPT_EXISTING),
			new LongOpt("ignore-existing", LongOpt.NO_ARGUMENT, null,
					OPT_IGNORE_EXISTING),
			new LongOpt("delete", LongOpt.NO_ARGUMENT, null, OPT_DELETE),
			new LongOpt("delete-excluded", LongOpt.NO_ARGUMENT, null,
					OPT_DELETE_EXCLUDED),
			new LongOpt("delete-after", LongOpt.NO_ARGUMENT, null,
					OPT_DELETE_AFTER),
			new LongOpt("ignore-errors", LongOpt.NO_ARGUMENT, null,
					OPT_IGNORE_ERRORS),
			new LongOpt("max-delete", LongOpt.REQUIRED_ARGUMENT, null,
					OPT_MAX_DELETE),
			new LongOpt("partial", LongOpt.NO_ARGUMENT, null, OPT_PARTIAL),
			new LongOpt("force", LongOpt.NO_ARGUMENT, null, OPT_FORCE),
			new LongOpt("numeric-ids", LongOpt.NO_ARGUMENT, null,
					OPT_NUMERIC_IDS),
			...
			new LongOpt("password-file", LongOpt.REQUIRED_ARGUMENT, null,
					OPT_PASSWORD_FILE),
			...
			new LongOpt("keystore", LongOpt.REQUIRED_ARGUMENT, null,
					OPT_KEYSTORE) };
	...
	public static void help(String progname, PrintStream out) {
		out.println(progname + "  (Jarsync "
				+ org.metastatic.rsync.version.VERSION + ")  protocol version "
				+ Constants.PROTOCOL_VERSION);
		out
				.println("Copyright (C) 2003  Casey Marshall <rsdio@metastatic.org>");
		...
		out
				.println("are welcome to redistribute it under certain conditions.  See the GNU");
		...
		out
				.println("SRC on single-colon remote HOST will be expanded by remote shell");
		out
				.println("SRC on server remote HOST may contain shell wildcards or multiple");
		out
				.println("  sources separated by space as long as they have same top-level");
		out.println();
		out.println("Options");
		...
		out
				.println(" -b, --backup                make backups (default ~ suffix)");
		out
				.println("     --backup-dir            make backups into this directory");
		out.println("     --suffix=SUFFIX         override backup suffix");
		out
				.println(" -u, --update                update only (don't overwrite newer files)");
		out.println(" -l, --links                 copy symlinks as symlinks");
		out
				.println(" -L, --copy-links            copy the referent of symlinks");
		out
				.println("     --copy-unsafe-links     copy links outside the source tree");
		out
				.println("     --safe-links            ignore links outside the destination tree");
		...
		out
				.println(" -D, --devices               preserve devices (root only)");
		out.println(" -t, --times                 preserve times");
		out
				.println(" -S, --sparse                handle sparse files efficiently");
		out
				.println(" -n, --dry-run               show what would have been transferred");
		out
				.println(" -W, --whole-file            copy whole files, no incremental checks");
		out.println("     --no-whole-file         turn off --whole-file");
		out
				.println(" -x, --one-file-system       don't cross filesystem boundaries");
		out
				.println(" -B, --block-size=SIZE       checksum blocking size (default 700)");
		out.println(" -e, --rsh=COMMAND           specify rsh replacement");
		out
				.println("     --rsync-path=PATH       specify path to rsync on the remote machine");
		out
				.println(" -C, --cvs-exclude           auto ignore files in the same way CVS does");
		out
				.println("     --existing              only update files that already exist");
		out
				.println("     --ignore-existing       ignore files that already exist on the receiving side");
		out
				.println("     --delete                delete files that don't exist on the sending side");
		out
				.println("     --delete-excluded       also delete excluded files on the receiving side");
		out
				.println("     --delete-after          delete after transferring, not before");
		out
				.println("     --ignore-errors         delete even if there are IO errors");
		out
				.println("     --max-delete=NUM        don't delete more than NUM files");
		out
				.println("     --partial               keep partially transferred files");
		out
				.println("     --force                 force deletion of directories even if not empty");
		out
				.println("     --numeric-ids           don't map uid/gid values by user/group name");
		out.println("     --timeout=TIME          set IO timeout in seconds");
		out
				.println(" -I, --ignore-times          don't exclude files that match length and time");
		out
				.println("     --size-only             only use file size when determining if a file should be transferred");
		out
				.println("     --modify-window=NUM     Timestamp window (seconds) for file match (default=0)");
		out
				.println(" -T  --temp-dir=DIR          create temporary files in directory DIR");
		out
				.println("     --compare-dest=DIR      also compare destination files relative to DIR");
		out
				.println(" -P                          equivalent to --partial --progress");
		out.println(" -z, --compress              compress file data");
		out
				.println("     --exclude=PATTERN       exclude files matching PATTERN");
		out
				.println("     --exclude-from=FILE     exclude patterns listed in FILE");
		out
				.println("     --include=PATTERN       don't exclude files matching PATTERN");
		out
				.println("     --include-from=FILE     don't exclude patterns listed in FILE");
		out.println("     --version               print version number");
		out
				.println("     --blocking-io           use blocking IO for the remote shell");
		out.println("     --no-blocking-io        turn off --blocking-io");
		out
				.println("     --stats                 give some file transfer stats");
		out
				.println("     --progress              show progress during transfer");
		out.println("     --password-file=FILE    get password from FILE");
		out
				.println("     --bwlimit=KBPS          limit I/O bandwidth, KBytes per second");
		out
				.println("     --ssl                   make socket connections over SSL (if available)");
		out
				.println("     -Joption                options to pass directly to java interpreter");
		out.println(" -h, --help                  show this help screen");
		out.println();
	}

	public static void version(String progname, PrintStream out) {
		out.print(progname + " (Jarsync "
		...
		out.println();
		out
				.println("Jarsync comes with NO WARRANTY, to the extent permitted by law.");
		out
				.println("You may redistribute copies of Jarsync under the terms of the GNU");
		out
				.println("General Public License.  See the file `COPYING' for details.");
	}
}
