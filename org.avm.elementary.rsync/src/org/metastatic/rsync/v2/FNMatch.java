 ...
 * 
 * <p>
 * I haven't tested this very thoroughly. I blindly replaced all the '/'
 * characters in the code with File.separatorChar. I tried to rework the logic
 * for C-language tests for '\0' correctly.
 * 
 * <p>
 * I would be eager to receive any bug reports at <a
			...
			case '*':
				...
				for (--p; string.length() != n; ++n) {
					if ((c == '[' || fold(string.charAt(n), flags) == c1)
							&& fnmatch(pattern.substring(p), string
									.substring(n), flags & ~FNM_PERIOD)) {
						return FNM_MATCH;
					}
				}
				return FNM_NOMATCH;
