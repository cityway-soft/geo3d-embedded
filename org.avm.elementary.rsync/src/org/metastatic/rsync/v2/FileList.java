/* FileList -- Send and recieve lists of files.
   $Id: FileList.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
...
public class FileList implements Constants {
	...
	private static final Logger logger = Logger.getInstance(FileList.class
			.getName());
	...
	public List createFileList(String[] argv, int off, int len)
		...
		Glob exclude = new Glob(options.exclude, false, System
	...
	private void sendFileEntry(FileInfo file) throws IOException {
		...
		out.write(flags);
		logger.debug("writing file name l1=" + l1 + " l2=" + l2 + " name="
				+ fname.substring(l1));
		if ((flags & SAME_NAME) != 0)
			out.write(l1);
		if ((flags & LONG_NAME) != 0)
