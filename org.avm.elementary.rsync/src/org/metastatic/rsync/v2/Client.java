/* Client -- main client process.
 $Id: Client.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
...
public class Client implements Constants {
	...
	public static final String PROGNAME = "rsync";

	private static final Logger logger = Logger
			.getInstance("org.metastatic.rsync");
	...
	public static int startClient(String[] argv, int optind) {
		...
		if (i > 0) {
			if (i + 1 < argv[optind].length()
					&& argv[optind].charAt(i + 1) == ':') {
			...
			if (argv[argv.length - 1].toLowerCase().startsWith(URL_PREFIX)) {
				remoteHost = argv[argv.length - 1].substring(URL_PREFIX
						.length());
				i = remoteHost.indexOf('/');
				if (i > 0) {
					path = remoteHost.substring(i + 1);
				...
				if (i > 0) {
					try {
						remotePort = Integer.parseInt(remoteHost
								.substring(i + 1));
					} catch (Exception x) {
						logger.error("bad port number.");
						// [DSU] System.exit(1);
	...
	public static int startSocketClient(String path, String[] argv, int optind) {
		logger.debug("starting socket client to " + remoteHost + ":"
				+ remotePort);
		if (path.startsWith("/")) {
			logger.error("remote path must start with a module name.");
			// [DSU] System.exit(1);
			throw new RuntimeException(
					"remote path must start with a module name.");
		}
		try {
			...
			in = new MultiplexedInputStream(new BufferedInputStream(socket
					.getInputStream()), false);
			out = new MultiplexedOutputStream(new BufferedOutputStream(socket
	...
	public static int startShellClient(String path, String[] argv, int optind) {
		...
		if (options.shell_cmd.equals("internal-ssh")) {
			...
			try {
				...
				in = new MultiplexedInputStream(new BufferedInputStream(p
						.getInputStream()), false);
				out = new MultiplexedOutputStream(new BufferedOutputStream(p
						.getOutputStream()), false);
				...
				if (remoteVersion < MIN_PROTOCOL_VERSION
					...
					logger
							.fatal("protocol version mismatch (is your shell clean?)");
					p.destroy();
					return 1;
				}
			} catch (IOException ioe) {
	...
	public static int clientRun(String[] argv, int optind) {
		try {
			...
			logger.debug("checksum seed="
					+ Util.toHexString(config.checksumSeed));
			if (remoteVersion >= 23)
			...
			FileList flist = new FileList(in, out, remoteVersion, false,
					options);
			...
			if (options.am_sender) {
				...
				Sender sender = new Sender(in, out, config, remoteVersion,
						false);
				...
				final Receiver recv = new Receiver(in, out, config,
						remoteVersion, false);
	...
	public static int readStats() {
		...
		if (options.do_stats) {
			...
			logger.warn("Total bytes written: " + stats.total_written
					+ " bytes");
		...
		if (options.verbose > 0 || options.do_stats) {
			...
			logger
					.warn("wrote "
							...
							+ " bytes  "
							+ df
									.format(((double) (stats.total_written + stats.total_read) / (0.5 + (double) ((t - starttime) / 1000L))))
							+ " bytes/sec");
			logger
					.warn("total size is "
							+ stats.total_size
							+ " bytes  speedup is "
							+ df
									.format((1.0 * stats.total_size)
											/ (double) (stats.total_written + stats.total_read)));
		}
		return 0;
	}

	public static void rsync(String[] argv) {
		...
		try {
			...
			throw new RuntimeException("could not create MD4 instance.");
		}
		config.strongSumLength = 2;
		config.blockLength = options.block_size;

		int ret = startClient(argv, optind);
	...
	public static void main(String[] argv) {
		...
		PatternLayout layout = new PatternLayout("%r %-5p [%c] %m%n");
		ConsoleAppender console = new ConsoleAppender(layout);
		root.addAppender(console);
		root.setPriority(Priority.DEBUG);
		root.info("Initialisation du logger user.dir = "
		...
		while (true) {
			if ((line = Util.readLine(in)) == null) {
				if (kludge_around_eof)
					// [DSU] System.exit(0);
					throw new RuntimeException(
							"did not receive server startup line.");
