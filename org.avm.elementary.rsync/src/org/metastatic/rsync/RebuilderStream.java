/* RebuilderStream: streaming file reconstructor.
   $Id: RebuilderStream.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
 ...
 * interface, which will write the data to the new file. Then call the
 * {@link update(Delta)} method for each {@link Delta} to be applied.
 * 
 * <p>
...
public class RebuilderStream {
	...
	public void update(Delta delta) throws IOException, ListenerException {
		...
		if (delta instanceof DataBlock) {
			e = new RebuilderEvent(((DataBlock) delta).getData(), delta
					.getWriteOffset());
		} else {
			if (basisFile == null)
				throw new IOException(
						"offsets found but no basis file specified");
			int len = Math.min(delta.getBlockLength(), (int) (basisFile
					.length() - ((Offsets) delta).getOldOffset()));
			if (len < 0)
				return;
