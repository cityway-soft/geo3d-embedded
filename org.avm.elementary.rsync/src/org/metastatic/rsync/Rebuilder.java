/* vim:set softtabstop=3 shiftwidth=3 tabstop=3 expandtab tw=72:
   $Id: Rebuilder.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
...
public class Rebuilder {
	 ...
	 *            The deltas to apply.
	 */
	public static void rebuild(OutputStream out, File oldFile, List deltas)
			throws IOException {
	...
	public static File rebuildFile(File oldFile, List deltas)
			throws IOException {
		File newFile = File.createTempFile(TMP_PREFIX, TMP_SUFFIX, oldFile
				.getParentFile());
			...
			if (o instanceof DataBlock) {
				...
				if (in == null) {
					throw new IOException(
							"original file does not exist or not readable");
				}
	...
	public static void rebuildFileInPlace(File file, List deltas)
		...
		for (Iterator i = deltas.iterator(); i.hasNext();) {
			...
			if (o instanceof Offsets) {
				...
				newFileLength = Math.max(newFileLength, ((Offsets) o)
						.getNewOffset()
						+ ((Offsets) o).getBlockLength());
			} else if (o instanceof DataBlock) {
				dataBlocks.add(o);
				newFileLength = Math.max(newFileLength, ((DataBlock) o)
						.getOffset()
	...
	private static boolean conflict(Offsets o1, Offsets o2) {
		return (o1.getNewOffset() >= o2.getOldOffset() && o1.getNewOffset() <= o2
				.getOldOffset()
				+ o2.getBlockLength())
				|| (o1.getNewOffset() + o1.getBlockLength() >= o2
						.getOldOffset() && o1.getNewOffset()
						+ o1.getBlockLength() <= o2.getOldOffset()
						+ o2.getBlockLength());
	}
