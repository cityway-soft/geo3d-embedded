/* TwoKeyMap -- Two-key Map implementation.
   $Id: TwoKeyMap.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
...
/**
 * <p>
 * This is a "double-keyed" mapping. The first key is a 16-bit integer, and the
 * second key is a variable-length byte array. With this, we can compute if a
 * given mapping is "probably" in the map using the first key, and compute
 * whether or not a mapping is definitely in the hashtable with the second key.
 * The rationale behind this is that the first key is trivial to compute and
 * that the second key is more difficult to compute but more unique.
 * </p>
 * 
 * <p>
 * Since the strong key can be a byte array of any length, then this "strong"
 * key can be shorter (and thus less unique) than the "weak" key. For this class
 * to work properly, the stronger key should be at least four bytes in length,
 * preferably longer.
 * </p>
 * 
 * <p>
 * The weak-key/strong-key method is inspired by (and is was written for) the
 * "hashtable" in the rsync algorithm, and has three levels of key search:
 * </p>
 * 
 * <ol>
 * <li>Test if the lower 2 bytes of the weak key (the positive part of a 32-bit
 * integer in Java) have been mapped to anything yet. This method always takes
 * O(1) time, and it is assumed that the weak key is trivial to compute.</li>
 * 
 * <li>Test if the entire weak key is mapped to anything. Since this class uses
 * a linked list to handle collisions where the lower 2 bytes are the same, this
 * method takes at most O(n) operations (also assuming that the weak key is
 * trivial to compute).</li>
 * 
 * <li>Test if both the weak and strong keys map to an Object. In addition to
 * the linked-list search of the second step, this involves a search of a
 * red-black tree, meaning that the upper-bound time complexity ranges from
 * O(lg(n)) to O(n). This works under the assumption that the strong key is some
 * sort of {@link MessageDigest}, and thus takes longer to compute.</li>
 * </ol>
 * 
 * <p>
 * With this method, we can determine if it is worth it to compute the strong
 * key if we have already computed the weak key.
 * </p>
 * 
 * <p>
...
public class TwoKeyMap implements java.io.Serializable, Map {
	...
	/**
	 * The sub-tables whose keys are the larger, stronger keys. The index of
	 * this array is the shorter, weaker key.
	 * 
	 * @since 1.1
	 */
	...
	/**
	 * A {@link java.util.Map.Entry} that contains another {@link java.util.Map}
	 * that is keyed with stronger, larger keys, and links to other SubTables
	 * whose {@link #key}'s lower four bytes are equivalent.
	 * 
	 * @since 1.1
	 * @version 1.1
	 * @author Casey Marshall
	...
	public class SubTable implements Map.Entry, java.io.Serializable {
		 ...
		 * @return The object mapped to by the given key, or null if there is no
		 *         such mapping.
		 */
		 ...
		 * 
		 * @return The key, a {@link java.lang.Integer} that maps to this entry.
		 * @since 1.1
		 */
		...
		public String toString() {
			return Integer.toHexString(key.intValue()) + " => "
					+ data.toString();
		}
	}
	 ...
	 * basically a wrapper around an array of bytes, providing methods important
	 * for using this class as the key in a Map, namely the {@link #hashCode()}
	 * and {@link #compareTo(java.lang.Object)} methods.
	 * 
	 * @since 1.1
	 * @version 1.0
	 * @author Casey Marshall
	...
	public class StrongKey implements java.io.Serializable, Comparable {
		 ...
		 * {@link java.util.Arrays#equals(byte[],byte[])} returns true for thier
		 * key arrays.
		 * 
		 * @since 1.0
		 * @throws java.lang.ClassCastException
		...
		/**
		 * Compare this object to another. This method returns an integer value
		 * less than, equal to, or greater than zero if this key is less than,
		 * equal to, or greater than the given key. This method will return
		 * <ul>
		...
		public int compareTo(Object o) {
			...
			if (sk.key == null)
				return -1;
			if (java.util.Arrays.equals(key, sk.getBytes()))
				return 0;
	 ...
	 * @throws java.lang.ClassCastException
	 *             If one of the keys in <code>m</code> is not a ChecksumPair.
	 * @throws java.lang.NullPointerException
	 *             If one of the keys in <code>m</code> is null.
	 */
	...
	/**
	 * Test if the map contains the lower two bytes of the weak key. This is the
	 * fastest, and least accurate <code>containsKey</code> method.
	 * 
	 * @since 1.1
	 * @param key
	 *            The key to check.
	 ...
	 * @since 1.1
	 * @param key
	 *            The {@link ChecksumPair} to use as the key.
	 * @param value
	 *            The value to map to.
	...
	/**
	 * Test if this map contains either the supplied weak key (if the argument
	 * is an Integer) or both the weak and strong keys (if the argument is a
	 * {@link ChecksumPair}).
	 * 
	 * @since 1.1
	 * @param key
	 *            The key to check.
	 * @return true If the map contains the given weak key (if <code>key</code>
	 *         is an Integer) or the given pair of keys (if <code>key</code>
	 *         is a {@link ChecksumPair}).
	 */
	...
	/**
	 * Get the object mapped to by the given key pair. The argument SHOULD be a
	 * {@link ChecksumPair}.
	 * 
	 * @since 1.1
	 * @param key
	 ...
	 * 
	 * @since 1.1
	 * @return <tt>true</tt> if <tt>o</tt> is an instance of this class and
	 *         if it contains the same sub-tables.
	 * @throws java.lang.ClassCastException
	 *             if <tt>o</tt> is not an instance of this class.
	 * @throws java.lang.NullPointerException
	 *             if <tt>o</tt> is null.
	...
	public Set keySet() {
		...
		fillArr: while (entries.hasNext()) {
			...
			for (Iterator keys = ((Map) t.getValue()).keySet().iterator(); keys
					.hasNext();) {
	...
	/**
	 * Put every entry in <code>m</code> in this map. This method will only
	 * work if the keys of <code>m</code> are of type {@link ChecksumPair}.
	 * 
	 * @since 1.1
	 * @param m
	 *            The mappings to put.
	 * @throws java.lang.ClassCastException
	 *             If every key in <code>m</code> is not a ChecksumPair.
	 * @throws java.lang.NullPointerException
	 *             If a key in <code>m</code> is null.
	 ...
	 * @return The removed object, if such a mapping existed. <code>null</code>
	 *         otherwise.
	 */
		...
		if (key instanceof ChecksumPair) {
			...
			while (t != null) {
				if (((Integer) t.getKey()).intValue() == pair.weak) {
					return ((Map) t.getValue()).remove(new StrongKey(
							pair.strong));
				}
			}
