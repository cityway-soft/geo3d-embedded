/* Checksum32 -- A simple, "rolling" checksum based on Adler32.
   $Id: Checksum32.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
 ...
 * of a block <em>X<sub>k</sub>...X<sub>l</sub></em>, then it is a simple
 * matter to compute the sum for <em>X<sub>k+1</sub>...X<sub>l+1</sub></em>.
 * 
...
public class Checksum32 implements RollingChecksum, Cloneable,
	...
	private Checksum32(Checksum32 that) {
		...
		this.l = that.l;
		this.k = that.k;
		this.block = (that.block != null) ? (byte[]) ((byte[]) that.block)
				.clone() : null;
		this.new_index = that.new_index;
		this.new_block = (that.new_block != null) ? (byte[]) ((byte[]) that.new_block)
				.clone()
				: null;
	}
	 ...
	 * <em>X<sub>k+1</sub>...X<sub>l+1</sub></em>. This is the preferred
