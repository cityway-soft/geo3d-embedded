/* MatcherStream: streaming alternative to Matcher.
   $Id: MatcherStream.java,v 1.2 2008/06/18 13:04:55 cvs Exp $
		...
		if (oldOffset != null) {
			if (ndx > config.blockLength) {
				DataBlock d = new DataBlock(count - ndx, buffer, 0, ndx
						- config.blockLength);
				Offsets o = new Offsets(oldOffset.longValue(), count
			...
			DataBlock d = new DataBlock(count - ndx, buffer, 0, buffer.length
		...
		if (hashtable.containsKey(weakSum.intValue())) {
			if (hashtable.containsKey(weakSum)) {
				...
				System.arraycopy(config.strongSum.digest(), 0, digest, 0,
						digest.length);
				return (Long) hashtable.get(new ChecksumPair(
						weakSum.intValue(), digest));
			}
		}
		return null;
	}
