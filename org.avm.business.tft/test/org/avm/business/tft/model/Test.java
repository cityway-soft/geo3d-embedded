...
public class Test extends TestCase {
	...
	private void process(AvmModel model) {
		...
		pts[rang] = new Point(rang + 1, rang + 1, "Point " + rang + 1, 0, 1, 0,
				rang + 1, 0, 0, 0, 0, 0);
		rang++;
		pts[rang] = new Point(rang + 1, rang + 1, "Point " + rang + 1, 0, 1, 0,
				rang + 1, 0, 0, 0, 0, 0);
		...
		COURSE crs = new COURSE(course.getId(), course.getIdu(),
				course.getNom(), course.getDepart(), course.getLigneIdu(),
				course.getLigneNom(), course.getAmplitude(),
				course.getChevauchement(), model.isHorsItineraire() ? 1 : 0);
		...
		for (int i = 0; i < points.length; i++) {
			...
			pnts[i] = new POINT(point.getId(), point.getIdu(), point.getNom(),
					...
					point.getLatitude(), (int) point.getDistance(),
					point.getCodeGirouette(), point.getArriveeTempsReel(), 0,
					point.getArriveeTheorique(), point.getAttente(),
					point.getRang(), point.isDesservi() ? 1 : 0, point.getItl());
		}
		crs.setPOINTS(pnts);
		sed.setCOURSE(crs);
	...
	public void testModel() {
		...
		Course course = new Course(123, 456, "crsblip", 0, "dest", "lig 9",
				111, "pcr 0", 0, 222, 333, 0);
		Point pts[] = new Point[3];
		pts[0] = new Point(0, 10, "HOTEL", 1, 2, 3, 4, 5, 6, 7, 8, 0);
		pts[0].setNomReduitGroupePoint("bilout");
		pts[1] = new Point(1, 20, "CHATELET", 11, 12, 13, 14, 15, 16, 17, 18, 0);
