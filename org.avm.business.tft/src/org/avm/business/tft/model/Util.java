	...
	public static String toJSONString(AvmModel model, String[] messages, int tft) {
		...
		if (course != null) {
			...
			for (int i = 0; i < points.length; i++) {
				...
				pnts[i] = new POINT(point.getId(), point.getIdu(),
						point.getNom(), point.getNomReduitGroupePoint(),
						point.getLongitude(), point.getLatitude(),
						(int) point.getDistance(), point.getCodeGirouette(),
						point.getArriveeTempsReel(), 0,
						point.getArriveeTheorique(), point.getAttente(),
						point.getRang(), point.isDesservi() ? 1 : 0,
						point.getItl());
			}
			crs.setPOINTS(pnts);
			sed.setCOURSE(crs);
