...
JNIEXPORT jint JNICALL Java_org_avm_device_protocol_comm_CommConnection_openImpl(
	...
	if ((flags & org_avm_device_protocol_comm_CommConnection_PARITY_ODD)
		...
		options.c_cflag |= PARENB;
		options.c_cflag &= ~PARODD;
		info("[DSU] comm : set parity even \n");
	} else {
		options.c_cflag &= ~PARENB;
		info("[DSU] comm : set parity none \n");
	}
	// bits/char
	...
	if ((flags & org_avm_device_protocol_comm_CommConnection_AUTORTS)
		...
		info("[DSU] comm : set hardware flow control \n");

	} else {
		options.c_cflag &= ~CRTSCTS;
		info("[DSU] comm : disable hardware flow control \n");
	}
	...
	info("[DSU] comm : set read timeout %d \n", timeout);

	tcsetattr(handle, TCSANOW, &options);
	info("[DSU] comm : port %d  opened \n", handle);
...
JNIEXPORT jint JNICALL Java_org_avm_device_protocol_comm_CommConnection_readImpl(
	...
	if (_timeout > 0) {
		...
		case -1:
			...
			break;
		case 0:
			debug("[DSU] comm : timeout handle = %d\n", handle);
			break;
		default:
