...
/**
 * The NDC class implements <i>nested diagnostic contexts</i> as defined by
 * Neil Harrison in the article "Patterns for Logging Diagnostic Messages" part
 * of the book "<i>Pattern Languages of Program Design 3</i>" edited by Martin
 * et al.
 * 
 * <p>
 ...
 * <em>current</em> thread only. NDCs of other threads remain unaffected.
 * 
 * <p>
 ...
 * <em>Cookies</em> are another source of distinctive information. To build an
 ...
 * 
 * <p>
 * <li>When entering a context, call <code>NDC.push</code>. As a side
 * effect, if there is no nested diagnostic context for the current thread, this
 ...
 * 
 * <p>
 * <li><b>When exiting a thread make sure to call {@link #remove NDC.remove()}</b>.
 * </ul>
 * 
 * <p>
 * There is no penalty for forgetting to match each <code>push</code>
 * operation with a corresponding <code>pop</code>, except the obvious
 * mismatch between the real application context and the context set in the NDC.
 * 
 * <p>
 * If configured to do so, {@link PatternLayout} instances automatically
 * retrieve the nested diagnostic context for the current thread without any
 * user intervention. Hence, even if a servlet is serving multiple clients
 * simultaneously, the logs emanating from the same code (belonging to the same
...
public class NDC {
	 ...
	 * exiting a thread. However, you must call <code>remove</code> sometime.
