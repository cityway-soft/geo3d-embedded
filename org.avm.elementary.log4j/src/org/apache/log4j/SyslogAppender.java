...
public class SyslogAppender extends AppenderSkeleton {
	...
	public SyslogAppender(Layout layout, String syslogHost, int syslogFacility,
			String bindAddress) {
		this(layout, syslogFacility);
		setSyslogHost(syslogHost, bindAddress);
	}
	...
	public void append(LoggingEvent event) {
		...
		if (sqw == null) {
			LogLog.error("No syslog host is set for SyslogAppedender named \""
					+ this.name + "\".");
			return;
		}
		...
		sqw.setLevel(event.priority.getSyslogEquivalent());
		try {
			sqw.write(buffer);
			/*
			 * String str = event.getThrowableStr(); if (str != null &
			 * str.length() > 0) {
			 * 
	...
	/**
	 * The <b>SyslogHost</b> option is the name of the the syslog host where
	...
	public void setSyslogHost(String syslogHost) {
		this.setSyslogHost(syslogHost, "0.0.0.0");
	}

	public void setSyslogHost(String syslogHost, String bindAddress) {
		this.sqw = new SyslogWriter(syslogHost, syslogFacility, bindAddress);
	 ...
	 * 
	 * <p>
	 * The <code>facilityName</code> parameter must be one of the strings
	 * KERN, USER, MAIL, DAEMON, AUTH, SYSLOG, LPR, NEWS, UUCP, CRON, AUTHPRIV,
	 * FTP, LOCAL0, LOCAL1, LOCAL2, LOCAL3, LOCAL4, LOCAL5, LOCAL6, LOCAL7. Case
	 * is unimportant.
	 * 
	 * @since 0.8.1
	 */
	public void setFacility(String facilityName) {
		...
		if (sqw != null) {
			// [DSU] sqw.setSyslogFacility(this.syslogFacility);
		}
	}
