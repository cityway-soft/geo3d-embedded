...
int vsi_din_open(struct inode *inode, struct file *filp) {
	struct vsi_device *device;
	struct vsi_din_data *data;

	device = container_of(inode->i_cdev, struct vsi_device, cdev);
	data = kmalloc(sizeof(struct vsi_din_data), GFP_KERNEL);
	data->value = 0;
	data->device = device;
	filp->private_data = data;
	pr_info("vsi: open device vsi%d \n", MINOR(device->cdev.dev));
	return 0;
}
