...
/**
 * <p>
 * A {@link Chain} represents a configured list of {@link Command}s that will
 * be executed in order to perform processing on a specified {@link Context}.
 * Each included {@link Command} will be executed in turn, until either one of
 * them returns <code>true</code>, one of the executed {@link Command}s
 * throws an exception, or the end of the chain has been reached. The
 * {@link Chain} itself will return the return value of the last {@link Command}
 * that was executed (if no exception was thrown), or rethrow the thrown
 * exception.
 * </p>
 * 
 * <p>
 ...
 * provided base class {@link org.apache.commons.chain.impl.ChainBase}) rather
...
public interface Chain extends Command {
	...
	/**
	 * <p>
	 * Add a {@link Command} to the list of {@link Command}s that will be
	 * called in turn when this {@link Chain}'s <code>execute()</code> method
	 * is called. Once <code>execute()</code> has been called at least once,
	 * it is no longer possible to add additional {@link Command}s; instead, an
	 ...
	 * {@link Command}s is encountered, or until one of the executed
	 * {@link Command}s returns <code>true</code> or throws an exception.</li>
	 * <li>Walk backwards through the {@link Command}s whose
	 * <code>execute()</code> methods, starting with the last one that was
	 * executed. If this {@link Command} instance is also a {@link Filter},
	 * call its <code>postprocess()</code> method, discarding any exception
	 * that is thrown.</li>
	 ...
	 * <code>true</code> if the last {@link Command} indicated that processing
	 * of this {@link Context} has been completed, or <code>false</code> if
	 * none of the called {@link Command}s returned <code>true</code>.</li>
	 * </ul>
	 * 
	 * @param context
