 ...
 * <em>Attribute-Property Transparency</em>. In other words, a value stored
 * via a call to <code>setFoo(value)</code> should be visible by calling
 * <code>get("foo")</code>, and a value stored via a call to
 * <code>put("foo", value)</code> should be visible by calling
 * <code>getFoo()</code>. If your {@link Context} implementation class
 * exhibits this featue, it becomes easier to reuse the implementation in
 * multiple environments, without the need to cast to a particular
 * implementation class in order to access the property getter and setter
 * methods.
 * </p>
 * 
 * <p>
