 ...
 * arbitrary objects.
 * <p>
 * A <tt>GenericKeyedObjectPool</tt> provides a number of configurable
 * parameters:
 * <ul>
 * <li> {@link #setMaxActive <i>maxActive</i>} controls the maximum number of
 * objects (per key) that can be borrowed from the pool at one time. When
 * non-positive, there is no limit to the number of objects that may be active
 * at one time. When {@link #setMaxActive <i>maxActive</i>} is exceeded, the
 * pool is said to be exhausted. </li>
 * <li> {@link #setMaxIdle <i>maxIdle</i>} controls the maximum number of
 * objects that can sit idle in the pool (per key) at any time. When negative,
 * there is no limit to the number of objects that may be idle at one time.
 * </li>
 * <li> {@link #setWhenExhaustedAction <i>whenExhaustedAction</i>} specifies the
 ...
 * which a {@link NoSuchElementException} will be thrown. If
 * {@link #setMaxWait <i>maxWait</i>} is non-positive, the {@link #borrowObject}
 * method will block indefinitely. </li>
 * </ul>
 * </li>
 ...
 * {@link KeyedPoolableObjectFactory#validateObject} method.) Objects that fail
 * to validate will be dropped from the pool. </li>
 * </ul>
 * <p>
 * Optionally, one may configure the pool to examine and possibly evict objects
 * as they sit idle in the pool. This is performed by an "idle object eviction"
 * thread, which runs asychronously. The idle object eviction thread may be
 ...
 * launched. </li>
 * <li> {@link #setMinEvictableIdleTimeMillis <i>minEvictableIdleTimeMillis</i>}
 * specifies the minimum amount of time that an object may sit idle in the pool
 * before it is eligable for eviction due to idle time. When non-positive, no
 * object will be dropped from the pool due to idle time alone. </li>
 * <li> {@link #setTestWhileIdle <i>testWhileIdle</i>} indicates whether or not
 ...
 * {@link KeyedPoolableObjectFactory}. A non-<code>null</code> factory must
 * be provided either as a constructor argument or via a call to
 * {@link #setFactory} before the pool is used.
 * </p>
 * 
 * @see GenericObjectPool
 * @author Rodney Waldhoff
 * @author Dirk Verbeeck
 * @version $Revision: 1.4 $ $Date: 2011/05/10 13:22:09 $
 */
public class GenericKeyedObjectPool extends BaseKeyedObjectPool implements
	...
	/**
	 * The default cap on the the maximum number of objects that can exists at
	 * one time.
	 * 
	 * @see #getMaxTotal
	 * @see #setMaxTotal
	 ...
	 * exhausted and the {@link #getWhenExhaustedAction "when exhausted" action}
	 * is {@link #WHEN_EXHAUSTED_BLOCK}.
	 * 
	 * @see #getMaxWait
	 * @see #setMaxWait
	 */
	...
	public GenericKeyedObjectPool() {
		this(null, DEFAULT_MAX_ACTIVE, DEFAULT_WHEN_EXHAUSTED_ACTION,
				DEFAULT_MAX_WAIT, DEFAULT_MAX_IDLE, DEFAULT_TEST_ON_BORROW,
	...
	/**
	 * Create a new <tt>GenericKeyedObjectPool</tt> using the specified
	 * values.
	 * 
	 * @param factory
	 *            the (possibly <tt>null</tt>)KeyedPoolableObjectFactory to
	 *            use to create, validate and destroy objects
	 */
	 ...
	 *            one time (per key) (see {@link #setMaxActive})
	 */
	public GenericKeyedObjectPool(KeyedPoolableObjectFactory factory,
			int maxActive) {
	...
	public GenericKeyedObjectPool(KeyedPoolableObjectFactory factory,
		...
		this(factory, maxActive, whenExhaustedAction, maxWait,
				DEFAULT_MAX_IDLE, DEFAULT_TEST_ON_BORROW,
				DEFAULT_TEST_ON_RETURN,
				DEFAULT_TIME_BETWEEN_EVICTION_RUNS_MILLIS,
				DEFAULT_NUM_TESTS_PER_EVICTION_RUN,
				DEFAULT_MIN_EVICTABLE_IDLE_TIME_MILLIS, DEFAULT_TEST_WHILE_IDLE);
	...
	public GenericKeyedObjectPool(KeyedPoolableObjectFactory factory,
		...
		this(factory, maxActive, whenExhaustedAction, maxWait,
				DEFAULT_MAX_IDLE, testOnBorrow, testOnReturn,
				DEFAULT_TIME_BETWEEN_EVICTION_RUNS_MILLIS,
				DEFAULT_NUM_TESTS_PER_EVICTION_RUN,
				DEFAULT_MIN_EVICTABLE_IDLE_TIME_MILLIS, DEFAULT_TEST_WHILE_IDLE);
	}
	 ...
	 *            {@link #setMaxWait})
	 * @param maxIdle
	 *            the maximum number of idle objects in my pool (per key) (see
	 *            {@link #setMaxIdle})
	 */
	public GenericKeyedObjectPool(KeyedPoolableObjectFactory factory,
	 ...
	 *            the pool before it is eligable for evcition (see
	...
	public GenericKeyedObjectPool(KeyedPoolableObjectFactory factory,
		...
		this(factory, maxActive, whenExhaustedAction, maxWait, maxIdle,
				GenericKeyedObjectPool.DEFAULT_MAX_TOTAL, testOnBorrow,
	 ...
	 *            {@link #setMaxIdle})
	 * @param maxTotal
	 *            the maximum number of objects that can exists at one time (see
	 *            {@link #setMaxTotal})
	 * @param testOnBorrow
	 *            whether or not to validate objects before they are returned by
	 *            the {@link #borrowObject} method (see {@link #setTestOnBorrow})
	...
	public GenericKeyedObjectPool(KeyedPoolableObjectFactory factory,
			int maxActive, byte whenExhaustedAction, long maxWait, int maxIdle,
			int maxTotal, boolean testOnBorrow, boolean testOnReturn,
			long timeBetweenEvictionRunsMillis, int numTestsPerEvictionRun,
			long minEvictableIdleTimeMillis, boolean testWhileIdle) {
		this(factory, maxActive, whenExhaustedAction, maxWait, maxIdle,
				maxTotal, GenericKeyedObjectPool.DEFAULT_MIN_IDLE,
	 ...
	 *            {@link #setMaxTotal})
	 * @param minIdle
	 *            the minimum number of idle objects to have in the pool at any
	 *            one time (see {@link #setMinIdle})
	 * @param testOnBorrow
	 *            whether or not to validate objects before they are returned by
	 *            the {@link #borrowObject} method (see {@link #setTestOnBorrow})
	 * @param testOnReturn
	...
	public GenericKeyedObjectPool(KeyedPoolableObjectFactory factory,
			int maxActive, byte whenExhaustedAction, long maxWait, int maxIdle,
			int maxTotal, int minIdle, boolean testOnBorrow,
	 ...
	 * 
	 * @param maxActive
	 *            The cap on the number of active instances from my pool (per
	 *            key). Use a negative value for an infinite number of
	 *            instances.
	 * @see #getMaxActive
	 */
	 ...
	 * 
	 * @param maxTotal
	 *            The cap on the total number of instances from my pool. Use a
	 *            non-positive value for an infinite number of instances.
	 * @see #getMaxTotal
	 */
	 ...
	 * when the pool is exhausted and the
	 ...
	 * before being returned by the {@link #borrowObject} method. If the object
	 * fails to validate, it will be dropped from the pool, and we will attempt
	 * to borrow another.
	 * 
	 * @see #setTestOnBorrow
	 */
	 ...
	 * When <tt>true</tt>, objects will be
	 * {@link org.apache.commons.pool.PoolableObjectFactory#validateObject validated}
	 * before being returned to the pool within the {@link #returnObject}.
	 * 
	 * @see #setTestOnReturn
	 */
	 ...
	 * of the idle objects will be tested per run.
	 * 
	 * @see #getNumTestsPerEvictionRun
	 * @see #setTimeBetweenEvictionRunsMillis
	 */
	...
	public synchronized void clear() {
		for (Iterator keyiter = _poolMap.keySet().iterator(); keyiter.hasNext();) {
			...
			for (Iterator it = list.iterator(); it.hasNext();) {
				try {
					_factory.destroyObject(key, ((ObjectTimestampPair) (it
							.next())).value);
				} catch (Exception e) {
	...
	/**
	 * Method clears oldest 15% of objects in pool. The method sorts the objects
	 * into a TreeMap and then iterates the first 15% for removal
	 */
	public synchronized void clearOldest() {
		...
		while (iter.hasNext() && itemsToRemove > 0) {
			...
			// objecttimestamppair
			// because it has the ordering with the timestamp value. Each value
			// that the
			// key references is the key of the list it belongs to.
			Object key = entry.getValue();
			ObjectTimestampPair pairTimeStamp = (ObjectTimestampPair) entry
					.getKey();
			LinkedList list = (LinkedList) _poolMap.get(key);
	...
	public synchronized void invalidateObject(Object key, Object obj)
			throws Exception {
		try {
			_factory.destroyObject(key, obj);
		} finally {
	 ...
	 * immediately commence a sustain cycle. If <i>populateImmediately</i> is
	 * <code>false</code>, the pool will be populated when the next schedules
	 * sustain task is run.
	 * 
	 * @param key -
	 *            The key to register for pool control.
	 * @param populateImmediately -
	 *            If this is <code>true</code>, the pool will start a sustain
	 *            cycle immediately.
	 */
		...
		for (int i = 0, m = getNumTests(); i < m; i++) {
			if (_poolMap.size() > 0) {
				...
				if (objIter.hasPrevious()) {
					ObjectTimestampPair pair = (ObjectTimestampPair) (objIter
							.previous());
	 ...
	 * maintain the minimum level of pooled objects.
	 * 
	 * @see #getMinIdle
	 * @see #setMinIdle
	 * @throws Exception
	 *             If there was an error whilst creating the pooled objects.
	 */
	private synchronized void ensureMinIdle() throws Exception {
		...
		if (_minIdle > 0) {
			...
			// Find out the total number of max active and max idle for that
			// class
	...
	/**
	 * Re-creates any needed objects to maintain the minimum levels of pooled
	 * objects for the specified key.
	 * 
	 * This method uses {@link #calculateDefecit} to calculate the number of
	 * objects to be created. {@link #calculateDefecit} can be overridden to
	...
	private synchronized void ensureMinIdle(Object key) throws Exception {
		...
		// Create required pool objects, if none to create, this loop will not
		// be run.
	...
	private int getNumTests() {
		if (_numTestsPerEvictionRun >= 0) {
			...
			return (int) (Math.ceil((double) _totalIdle
	...
	/**
	 * This returns the number of objects to create during the pool sustain
	 * cycle. This will ensure that the minimum number of idle connections is
	 * maintained without going past the maxPool value.
	 * <p>
	 * This method has been left public so derived classes can override the way
	 * the defecit is calculated. ie... Increase/decrease the pool size at
	 * certain times of day to accomodate for usage patterns.
	 * 
	 * @param key -
	 *            The key of the pool to calculate the number of objects to be
	 *            re-created
	 ...
	 * is eligable for eviction by the idle object evictor (if any). When
	 * non-positive, no objects will be evicted from the pool due to idle time
	 * alone.
	 * 
	 * @see #setMinEvictableIdleTimeMillis
