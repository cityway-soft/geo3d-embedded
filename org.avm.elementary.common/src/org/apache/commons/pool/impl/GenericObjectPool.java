 ...
 * objects that can be borrowed from the pool at one time. When non-positive,
 * there is no limit to the number of objects that may be active at one time.
 * When {@link #setMaxActive <i>maxActive</i>} is exceeded, the pool is said to
 * be exhausted. </li>
 * <li> {@link #setMaxIdle <i>maxIdle</i>} controls the maximum number of
 * objects that can sit idle in the pool at any time. When negative, there is no
 * limit to the number of objects that may be idle at one time. </li>
 ...
 * {@link PoolableObjectFactory#validateObject} method.) Objects that fail to
 * validate will be dropped from the pool. </li>
 * </ul>
 * <p>
...
public class GenericObjectPool extends BaseObjectPool implements ObjectPool {
	...
	/**
	 * Idle object evition Timer. Shared between all {@link GenericObjectPool}s
	 * and {@link GenericKeyedObjectPool} s.
	 */
	 ...
	 *            the minimum number of milliseconds an object can sit idle in
	 *            the pool before it is eligable for evcition with the extra
	 ...
	 * thread (if active) spawns new objects. (Note no objects are created when:
	...
	/**
	 * Sets the minimum amount of time an object may sit idle in the pool before
	 * it is eligable for eviction by the idle object evictor (if any), with the
	...
	public synchronized void clear() {
		...
		for (Iterator it = _pool.iterator(); it.hasNext();) {
			try {
				_factory
						.destroyObject(((ObjectTimestampPair) (it.next())).value);
			} catch (Exception e) {
	...
	public synchronized void evict() throws Exception {
		...
		if (!_pool.isEmpty()) {
			...
			for (int i = 0, m = getNumTests(); i < m; i++) {
				...
				final ObjectTimestampPair pair = (ObjectTimestampPair) (iter
						.previous());
	...
	synchronized String debugInfo() {
		...
		while (it.hasNext()) {
			...
			buf.append("\t").append(pair.value).append("\t").append(
					time - pair.tstamp).append("\n");
		}
		return buf.toString();
	}
	...
	/**
	 * The minimum amount of time an object may sit idle in the pool before it
	 * is eligable for eviction by the idle object evictor (if any), with the
	 * extra condition that at least "minIdle" amount of object remain in the
	 * pool. When non-positive, no objects will be evicted from the pool due to
	 * idle time alone.
	 * 
