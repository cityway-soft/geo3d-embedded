...
/**
 * A simple, {@link java.util.Stack Stack}-based {@link KeyedObjectPool}
 * implementation.
 * <p>
 * Given a {@link KeyedPoolableObjectFactory}, this class will maintain a
 * simple pool of instances. A finite number of "sleeping" or inactive instances
 * is enforced, but when the pool is empty, new instances are created to support
 * the new load. Hence this class places no limit on the number of "active"
 * instances created by the pool, but is quite useful for re-using
 * <tt>Object</tt>s without introducing artificial limits.
 * 
 * @author Rodney Waldhoff
 * @version $Revision: 1.4 $ $Date: 2011/05/10 13:22:09 $
 */
public class StackKeyedObjectPool extends BaseKeyedObjectPool implements
		KeyedObjectPool {
	/**
	 * Create a new pool using no factory. Clients must first populate the pool
	 * using {@link #returnObject(java.lang.Object,java.lang.Object)} before
	 * they can be {@link #borrowObject(java.lang.Object) borrowed}.
	 */
	public StackKeyedObjectPool() {
		this((KeyedPoolableObjectFactory) null, DEFAULT_MAX_SLEEPING,
	...
	public StackKeyedObjectPool(int max) {
		this((KeyedPoolableObjectFactory) null, max,
				DEFAULT_INIT_SLEEPING_CAPACITY);
	}
	 ...
	 *            cap on the number of "sleeping" instances in the pool
	 * @param init
	 *            initial size of the pool (this specifies the size of the
	...
	/**
	 * Create a new <tt>SimpleKeyedObjectPool</tt> using the specified
	 * <i>factory</i> to create new instances.
	 * 
	 * @param factory
	 *            the {@link KeyedPoolableObjectFactory} used to populate the
	 *            pool
	 */
	...
	/**
	 * Create a new <tt>SimpleKeyedObjectPool</tt> using the specified
	 * <i>factory</i> to create new instances. capping the number of "sleeping"
	 * instances to <i>max</i>
	 * 
	 * @param factory
	 *            the {@link KeyedPoolableObjectFactory} used to populate the
	 *            pool
	 ...
	 * Create a new <tt>SimpleKeyedObjectPool</tt> using the specified
	 * <i>factory</i> to create new instances. capping the number of "sleeping"
	 * instances to <i>max</i>, and initially allocating a container capable of
	 * containing at least <i>init</i> instances.
	 * 
	 * @param factory
	 *            the {@link KeyedPoolableObjectFactory} used to populate the
	 *            pool
	...
	public StackKeyedObjectPool(KeyedPoolableObjectFactory factory, int max,
		...
		_initSleepingCapacity = (init < 1 ? DEFAULT_INIT_SLEEPING_CAPACITY
				: init);
		_pools = new HashMap();
	...
	public synchronized Object borrowObject(Object key) throws Exception {
		...
		if (null == stack) {
			stack = new Stack();
			stack
					.ensureCapacity(_initSleepingCapacity > _maxSleeping ? _maxSleeping
	...
	public synchronized String toString() {
		...
		buf.append(" contains ").append(_pools.size()).append(
				" distinct pools: ");
		Iterator it = _pools.keySet().iterator();
		while (it.hasNext()) {
			Object key = it.next();
	...
	/**
	 * The default initial size of the pool (this specifies the size of the
	 * container, it does not cause the pool to be pre-populated.)
	 */
